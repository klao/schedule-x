...
const createCalendarState = (calendarConfig, timeUnitsImpl, selectedDate) => {
    var _a;
    const _view = signal(((_a = calendarConfig.views.find((view) => view.name === calendarConfig.defaultView)) === null || _a === void 0 ? void 0 : _a.name) || calendarConfig.views[0].name);
    const view = computed(() => {
        return _view.value;
    });
...
function TimeGridEvent({ calendarEvent, dayBoundariesDateTime, isCopy, setMouseDown, }) {
    ...
    return (jsxs(Fragment, { children: [jsx("div", { id: isCopy ? getTimeGridEventCopyElementId(calendarEvent.id) : undefined, "data-event-id": calendarEvent.id, onClick: handleOnClick, onKeyDown: handleKeyDown, onMouseDown: handlePointerDown, onMouseUp: handlePointerUp, onTouchStart: handlePointerDown, onTouchEnd: handlePointerUp, className: classNames.join(' '), tabIndex: 0, style: {
                    ...
                    padding: customComponent ? '0' : undefined,
                                ...
                                jsxs("div", { className: "sx__time-grid-event-people", children: [jsx(LocationPinIcon, { strokeColor: eventCSSVariables.iconStroke }), calendarEvent.location] }))] })), $app.config.plugins.resize &&
                            !((_b = calendarEvent._options) === null || _b === void 0 ? void 0 : _b.disableResize) && (jsx("div", { className: 'sx__time-grid-event-resize-handle', onMouseDown: startResize }))] }) }), eventCopy && (jsx(TimeGridEvent, { calendarEvent: eventCopy, isCopy: true, setMouseDown: setMouseDown }))] }));
}
...
function DateAxis({ week }) {
    ...
    return (jsx(Fragment, { children: jsx("div", { className: "sx__week-grid__date-axis", children: week.map((date) => (jsxs("div", { className: getClassNames(date), "data-date": toDateString$1(date), children: [jsx("div", { className: "sx__week-grid__day-name", children: getDayNameShort(date, $app.config.locale) }), jsx("div", { className: "sx__week-grid__date-number", children: date.getDate() })] }))) }) }));
}
...
function DateGridEvent({ calendarEvent, gridRow, isCopy, }) {
    ...
    return (jsxs(Fragment, { children: [jsxs("div", { id: isCopy ? getTimeGridEventCopyElementId(calendarEvent.id) : undefined, tabIndex: 0, "aria-label": calendarEvent.title +
                ...
                }, children: [!customComponent && (jsxs(Fragment$1, { children: [hasOverflowLeft && (jsx("div", { className: 'sx__date-grid-event--left-overflow', style: overflowStyles })), jsx("span", { className: "sx__date-grid-event-text", children: calendarEvent.title }), hasOverflowRight && (jsx("div", { className: 'sx__date-grid-event--right-overflow', style: overflowStyles }))] })), $app.config.plugins.resize &&
...
function MonthAgendaDay({ day, isActive, setActiveDate, }) {
    ...
    return (jsxs("button", { className: dayClasses.join(' '), onClick: handleClick, "aria-label": getLocalizedDate(day.date, $app.config.locale), tabIndex: hasFocus(day) ? 0 : -1, "data-agenda-focus": hasFocus(day) ? 'true' : undefined, onKeyDown: handleKeyDown, children: [jsx("div", { children: jsDate.getDate() }), jsx("div", { className: "sx__month-agenda-day__event-icons", children: day.events.slice(0, 3).map((event) => (jsx("div", { style: {
